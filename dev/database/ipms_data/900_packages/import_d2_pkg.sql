...
create or replace package import_d2_pkg as
	...
	procedure merge_into_import_d2(p_import_id in nvarchar2);

	--update existing projects and import READY
	--procedure loop_for_all_updated_d1(p_import_id in nvarchar2);
    
    procedure loop_for_all_updated_d2(p_import_id in nvarchar2);

end;
/****************************************/
create or replace package body import_d2_pkg as
			...
			from (
				...
				from (
					...
					from (
						...
						from import_d2
						where trg_code is not null and  (trg_code <> 'TA spanning')--Filter applied for Unique constraint Issue(Two Trg with same name)
	...
	begin
		...
		import_d2_pkg.merge_into_import_d2(v_id);
		import_d2_pkg.loop_for_all_updated_d2(v_id);

	end;

	/*************************************************************************/
	procedure loop_for_all_updated_d2(p_import_id in nvarchar2) as
		...
		v_ordering number;
	begin
		select id into v_program_id from program where code='RESERVED-PT-D2-PRJ';

		for rr in (
					select prj.area_code, prj.code,prj.id, d2.*
					from import_d2 d2
					left join project prj on (to_char(d2.project_id)=prj.code and prj.area_code='D2-PRJ')-- we are updating only D2
					where d2.import_id=p_import_id
			...
			if rr.status_code='READY' or rr.id is not null then --take only READY or already imported
				begin
					...
					exception when no_data_found then
						...
						values ('D2-PRJ.'||rr.project_phase_no,rr.project_phase,1,v_ordering) returning code into v_phase_code;
					end;
					...
					if rr.status_code='READY' and rr.id is null then
					...
					elsif rr.id is not null and rr.area_code='D2-PRJ' then
						update project set--Change on PROMIS-608 to reduce impacted imported fields
						...
						where id=rr.id and code=rr.project_id and state_code in (0,1)
						returning id into v_prj_id;
    ...
    /*************************************************************************/
	procedure merge_into_import_d2(p_import_id in nvarchar2) as
		v_rowcount number;
		v_timestamp_log import.timestamp_log%type;
		v_sqlerrm nvarchar2(4000);
	begin

		merge into import_d2 dest
		using (
			select
				project_id,
		...
		log_pkg.log('import_d2_pkg.merge_into_import_d2',v_timestamp_log,'Updated.');

	exception when others then
		v_sqlerrm:=substr(sqlerrm,1,4000);
		log_pkg.log('import_d2_pkg.merge_into_import_d2', nvl(v_timestamp_log,'p_import_id='||p_import_id||';v_rowcount='||v_rowcount), v_sqlerrm);
