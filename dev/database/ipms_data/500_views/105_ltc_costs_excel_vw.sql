create or replace view ltc_costs_excel_vw as
--with all_costs as (
...
with
	vwpar as (
		select
			ltc_report_pkg.get_project_id as par_project_id
		from dual
	)
select 
	cst.project_id,
...
and cst.project_id = decode(vwpar.par_project_id,null,cst.project_id,vwpar.par_project_id)--decode should be faster
