create or replace view ltc_imported_costs_vw as
select
...
inner join ltc_costs_vw cst on (cst.project_id=nvl(tl.project_id, sb.src_project_id))
left join timeline_wbs w on (w.timeline_id=tl.id and to_nchar(w.study_id)=cst.study_id)
where tl.type_code in ('RAW','SND')
