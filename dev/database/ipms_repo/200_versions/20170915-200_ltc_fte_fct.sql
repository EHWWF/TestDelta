		...
		select
			...
			decode
				(
					...
					1,ltce.fct_y2_cost,
					2,ltce.fct_y3_cost
				) as fct_det, --Forecast Deterministing for first 3 years
			decode
				(
					year_nr_table.year_nr,
					0,ltce.lt_y1_cost,
			...
			decode
				(
					...
					1,ltce.fct_prob_y1_cost,
					2,ltce.fct_prob_y1_cost
				) as fct_prob --Forecast Probabilistic for first 3 years
...
select
	...
	decode(ltce.scope_code,'INT',decode(nvl(ltce.function_cost_rate,0),0,null,(ltcc.ltc_det/ltce.function_cost_rate)),null) as fte_calc, --Calculated based on Function Cost_Rate
	--wmud.max_ltc_update_date
...
--join with_range_max_update_date wmud on (wmud.project_id=ltcprj.project_id and ltcprj.ltc_process_id=wmud.ltc_process_id and rank_max_update_date=1)
...
where ltce.is_discrepancy=0;
create table cost_ltc_fte_fct as select * from cost_ltc_fte_fct_vw where 1=0;
create unique index cost_ltc_fte_fct_ui on cost_ltc_fte_fct (ltci_id,estimate_id,period_id);
alter table promis_task add ltc_process_id number;
alter table promis_task add project_id nvarchar2(20);
alter table promis_task add ltc_tag_id number;