...
where demand>0
group by rsc.project_id,rsc.study_id,rsc.project_activity_id,sf.function_code,rsc.subfunction_code,t.period_id,rsc.decision_start,rsc.commit_state
) x
cross join (select 1 from dual);

grant select on resource_ged_fct to mycsd;