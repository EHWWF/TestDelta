...
left join ipms_repo.period_dim t on t.year=extract(year from tpp.approval_date) and t.month_of_year=extract(month from tpp.approval_date);

grant select on tpp_fct to mycsd;