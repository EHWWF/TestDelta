...
public class EstimatesProcessBean extends IPMSBean implements Serializable {

    private static final String IT_LEP = "LatestEstimatesProcessViewIterator";

    public EstimatesProcessBean() {
    }
    ...
    public void onDelete(ActionEvent event) {
        ...
        LatestEstimatesProcessViewRow lepRow = (LatestEstimatesProcessViewRow) ViewUtils.getCurrentRow(IT_LEP);

        // loop through selected rows
        for (Iterator it = keySet.iterator(); it.hasNext();) {
            List<Key> keys = (List<Key>) it.next();
            LatestEstimatesProjectViewRow prj =
                (LatestEstimatesProjectViewRow) lepRow.getLatestEstimatesProjectView().getRow(keys.get(0));
            prj.remove();
        }
    ...
    public DnDAction onDrop(DropEvent dropEvent) {
        ...
        ViewUtils.getIteratorBinding("LatestEstimatesProjectViewIterator").executeQuery();
    ...
    public void onProjectTabDisclose(DisclosureEvent disclosureEvent) {
        ...
        if (detItem.isDisclosed()) {
            ...
            if (detItem.getId().equals("diDev")) {
                ((ProgramView) ViewUtils.getIteratorBinding("DevProgramViewIterator").getViewObject()).setSearchVcName("LatestEstimatesDev");
            } else if (detItem.getId().equals("diPrdMnt")) {
                ((ProgramView) ViewUtils.getIteratorBinding("PrdMntProgramViewIterator").getViewObject()).setSearchVcName("LatestEstimatesPrdMnt");
            }
    ...
    public void onPrefillProcessChange(ValueChangeEvent event) {
        ...
        ViewUtils.getIteratorBinding("PreviousTagLatestEstimatesProcessViewIterator").setCurrentRowIndexInRange((Integer) event.getNewValue());

        DCIteratorBinding srcPrjIt = ViewUtils.getIteratorBinding("PreviousTagLatestEstimatesProjectViewIterator");
        srcPrjIt.executeQuery();

        RowSetIterator rsi = srcPrjIt.getViewObject().createRowSetIterator(null);

        while (rsi.hasNext()) {
            lepRow.add((LatestEstimatesProjectViewRow) rsi.next());
        }
    ...
    public Boolean getContainsProjects() {
        ...
        if ((LatestEstimatesProjectViewRow) lepRow.getLatestEstimatesProjectView().first() == null) {
            return false;
        } else {
            return true;
        }
